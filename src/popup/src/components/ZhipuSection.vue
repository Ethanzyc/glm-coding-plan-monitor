...
import type { AccountUsageData, QuotaItem } from '../../../shared/types'
...
function hasHistoryData(acc: AccountUsageData): boolean {
  return !!(acc.modelHistory1d?.length || acc.modelHistory7d?.length || acc.modelHistory30d?.length ||
    acc.mcpHistory1d?.length || acc.mcpHistory7d?.length || acc.mcpHistory30d?.length)
}

function hasPerformanceData(acc: AccountUsageData): boolean {
  return !!(acc.performanceHistory7d?.length || acc.performanceHistory15d?.length || acc.performanceHistory30d?.length)
}
